...
const SHEETS_TOOLS = [
  ...
  // Formatting (batchUpdate) tools
  { name: "format_cells", description: "Apply formatting to a range of cells — bold, italic, underline, font_size, font_family, foreground_color, background_color, horizontal_alignment, vertical_alignment, wrap_strategy, number_format. CRITICAL: use A1 notation for range (e.g. '1:1' for entire row 1). tab_name is the exact tab name. Always call get_spreadsheet_info first.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, range: { type: Type.STRING }, bold: { type: Type.BOOLEAN }, italic: { type: Type.BOOLEAN }, underline: { type: Type.BOOLEAN }, font_size: { type: Type.NUMBER }, font_family: { type: Type.STRING }, foreground_color: { type: Type.STRING }, background_color: { type: Type.STRING }, horizontal_alignment: { type: Type.STRING }, vertical_alignment: { type: Type.STRING }, wrap_strategy: { type: Type.STRING }, number_format_type: { type: Type.STRING }, number_format_pattern: { type: Type.STRING } }, required: ["spreadsheet_id", "tab_name", "range"] } },
  { name: "format_row", description: "Format an entire row by row number (1-indexed). Use for 'bold the header row' requests. row_number=1 = first row.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, row_number: { type: Type.NUMBER }, bold: { type: Type.BOOLEAN }, italic: { type: Type.BOOLEAN }, font_size: { type: Type.NUMBER }, foreground_color: { type: Type.STRING }, background_color: { type: Type.STRING }, horizontal_alignment: { type: Type.STRING } }, required: ["spreadsheet_id", "tab_name", "row_number"] } },
  { name: "freeze_rows", description: "Freeze top N rows. row_count=1 freezes just the header. row_count=0 unfreezes.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, row_count: { type: Type.NUMBER } }, required: ["spreadsheet_id", "tab_name", "row_count"] } },
  { name: "freeze_columns", description: "Freeze left N columns. column_count=1 freezes col A.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, column_count: { type: Type.NUMBER } }, required: ["spreadsheet_id", "tab_name", "column_count"] } },
  { name: "set_column_width", description: "Set column width in pixels. Columns are 0-indexed (A=0). end_column_index is exclusive.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, start_column_index: { type: Type.NUMBER }, end_column_index: { type: Type.NUMBER }, pixel_size: { type: Type.NUMBER } }, required: ["spreadsheet_id", "tab_name", "start_column_index", "end_column_index", "pixel_size"] } },
  { name: "set_row_height", description: "Set row height in pixels. Rows are 0-indexed (row 1 = index 0). end_row_index is exclusive.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, start_row_index: { type: Type.NUMBER }, end_row_index: { type: Type.NUMBER }, pixel_size: { type: Type.NUMBER } }, required: ["spreadsheet_id", "tab_name", "start_row_index", "end_row_index", "pixel_size"] } },
  { name: "auto_resize_columns", description: "Auto-fit column widths to content. Use after writing data.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, start_column_index: { type: Type.NUMBER }, end_column_index: { type: Type.NUMBER } }, required: ["spreadsheet_id", "tab_name"] } },
  { name: "merge_cells", description: "Merge cells. merge_type: MERGE_ALL, MERGE_COLUMNS, MERGE_ROWS. Set unmerge=true to unmerge.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, range: { type: Type.STRING }, merge_type: { type: Type.STRING }, unmerge: { type: Type.BOOLEAN } }, required: ["spreadsheet_id", "tab_name", "range"] } },
  { name: "sort_sheet_range", description: "Sort rows by a column. sort_column is 0-indexed (A=0). sort_order: ASCENDING or DESCENDING.", parameters: { type: Type.OBJECT, properties: { spreadsheet_id: { type: Type.STRING }, tab_name: { type: Type.STRING }, range: { type: Type.STRING }, sort_column: { type: Type.NUMBER }, sort_order: { type: Type.STRING } }, required: ["spreadsheet_id", "tab_name", "range", "sort_column", "sort_order"] } },
];
...
12. To share a document for inline preview (Google Drive files, PDFs), use a fenced code block with language "document_preview". Content must be JSON: { "title": "Document Name", "url": "https://drive.google.com/file/d/.../view", "fileType": "drive"|"pdf"|"gdoc"|"gsheet"|"gslides"|"image", "downloadUrl": "optional download link" }. The user will see a clickable card that opens the document in a side panel.
